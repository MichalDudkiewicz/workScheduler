...
BOOST_AUTO_TEST_CASE(InputIncorrectPathCase)
{
  input::loadPositions();
  BOOST_REQUIRE_THROW(
    input::employeeRepository(
      "../../../dataset/some_directory/employeeRepository.csv"),
    std::logic_error);
  BOOST_REQUIRE_THROW(
    input::teamRepository("../../../dataset/some_directory/teamRepository.csv"),
    std::logic_error);
  input::employeeRepository("../../../dataset/.test/employeeRepository.csv");
  input::teamRepository("../../../dataset/.test/teamRepository.csv");
  BOOST_REQUIRE_THROW(
    input::teamSchedule("../../../dataset/some_directory/teamSchedule.csv"),
    std::logic_error);
  BOOST_REQUIRE_THROW(input::desiredSchedule(
                        "../../../dataset/some_directory/desiredSchedule.csv"),
                      std::logic_error);
}
...
BOOST_AUTO_TEST_CASE(InputIncorrectEmployeeTeamCase)
{
  ...
  BOOST_REQUIRE_THROW(
    input::desiredSchedule("../../../dataset/.test/desiredSchedule.csv"),
    std::logic_error);
}
...
BOOST_AUTO_TEST_CASE(SchedulesCreationCase)
{
  ...
  BOOST_REQUIRE_EQUAL(EmployeeRepository::getInstance()
                        .get(2)
                        ->getFactor()
                        ->getAvailability()
                        .getDesiredSchedule()
                      ...
                      "day: 28, 0 - 24\n"
                      "day: 29, 0 - 24\n");
  BOOST_REQUIRE_EQUAL(TeamRepository::getInstance().get("S2")->teamInfo(),
                      "Team S2\nPositions required: doctor, driver (special), "
                      "rescuer (special), \n"
                      "Team shifts: \n"
                      "Monday: day: 1, 7 - 19\n"
                      "Tuesday: day: 2, 7 - 19\n"
...
BOOST_AUTO_TEST_CASE(OutputIncorrectPathCase)
{
  ...
  BOOST_REQUIRE_THROW(
    output::employeeRepository(
      "../../../dataset/some_directory/employeeRepository.csv"),
    std::logic_error);
  BOOST_REQUIRE_THROW(output::teamRepository(
                        "../../../dataset/some_directory/teamRepository.csv"),
                      std::logic_error);
  BOOST_REQUIRE_THROW(
    output::teamSchedule("../../../dataset/some_directory/teamSchedule.csv"),
    std::logic_error);
  BOOST_REQUIRE_THROW(output::desiredSchedule(
                        "../../../dataset/some_directory/desiredSchedule.csv"),
                      std::logic_error);
  BOOST_REQUIRE_THROW(
    output::schedule("../../../dataset/some_directory/schedule.csv"),
    std::logic_error);
}
