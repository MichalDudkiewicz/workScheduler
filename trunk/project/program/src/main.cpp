...
//    sql::PreparedStatement *prep_stmt;
    ...
    while (res->next()) {
        TeamManager::getInstance().get(res->getString(1))->addPosition(PositionManager::getInstance().get(res->getInt(2)));
    }
    ...
    while (res->next())
    {
        for (unsigned int i = 1; i <= 7; i ++)
        {
            if(!(res->isNull(2 * i) or res->isNull(2 * i + 1)))
                TeamManager::getInstance().get(res->getString(1))->addShift(res->getInt(2 * i),res->getInt(2 * i + 1),i);
        }
    }


        res = stmt->executeQuery("SELECT ID, name, wage, points, priority, nonresident FROM employee_repository");

        while (res->next()) {
            EmployeeManager::getInstance().add(res->getInt(1), res->getString(2));
            EmployeeManager::getInstance().get(res->getInt(1))->setHourlyWage(res->getInt(3));
            EmployeeManager::getInstance().get(res->getInt(1))->getFactor()->getRules().setPoints(res->getInt(4));
            EmployeeManager::getInstance().get(res->getInt(1))->getFactor()->getRules().changeType(res->getInt(5));
            EmployeeManager::getInstance().get(res->getInt(1))->getFactor()->getRules().setNonresident(
                    boost::lexical_cast<bool>(res->getInt(6)));
        }

    res = stmt->executeQuery("SELECT employee_id, team FROM employee_team");
    while (res->next()) {
        EmployeeManager::getInstance().get(res->getInt(1))->getFactor()->getAuthorisation().addTeam(TeamManager::getInstance().get(res->getString(2)));
    }
        ...
        while (res->next()) {
            if (res->getInt(2) < res->getInt(1)) {
                EmployeeManager::getInstance().get(res->getInt(1))->getFactor()->getRelationship().addEnemy(
                        EmployeeManager::getInstance().get(res->getInt(2)));
            }
        }
        ...
        while (res->next()) {
            EmployeeManager::getInstance().get(res->getInt(1))->getFactor()->getAuthorisation().addPosition(PositionManager::getInstance().get(res->getInt(2)));
        }
    ...
    while (res->next()) {
        for(unsigned int i = 3; i < number_of_columns; i+=2)
        {
            if(!(res->isNull(i) or res->isNull(i + 1)))
                EmployeeManager::getInstance().get(res->getInt(1))->getFactor()->getAvailability().getDesiredSchedule().addShift(res->getInt(i), res->getInt(i+1), res->getInt(2));
        }
    }
