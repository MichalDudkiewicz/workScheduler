...
input::loadPositions()
{
  ...
  std::shared_ptr<Position> medicalRecorder =
    std::make_shared<MedicalRecorder>();
...
void
input::employeeRepository(const std::string& path)
{
    ...
    if (columnNumber == 8) {
      ...
      if (rowNumber > 0) {
        ...
        std::vector<unsigned int> positions =
          cellToRawValues<unsigned int>(row[6], ';');
        ...
        std::vector<unsigned int> enemies =
          cellToRawValues<unsigned int>(row[7], ';');
...
void
input::teamSchedule(const std::string& path)
{
  std::vector<std::string> row;
  unsigned int columnNumber = 0;
  unsigned int rowNumber = 0;
    ...
    if (columnNumber == 8) {
      ...
      if (rowNumber > 0) {
        for (unsigned int i = 0; i < 7; ++i) {
          ...
          std::vector<unsigned int> shiftHours =
            cellToRawValues<unsigned int>(hoursChain, '-');
...
void
input::teamRepository(const std::string& path)
{
  ...
  while (teamRepositoryS.good()) {
    ...
    if (columnNumber == 2) {
      ...
      if (rowNumber > 0) {
        ...
        std::vector<unsigned int> positions =
          cellToRawValues<unsigned int>(row[1], ';');
...
void
input::desiredSchedule(const std::string& path)
{
        ...
        for (auto& c : row) {
          if (day > 0) {
            std::vector<std::string> shifts =
              cellToRawValues<std::string>(c, ';');
            for (auto& shift : shifts) {
              std::vector<unsigned int> shiftHours =
                cellToRawValues<unsigned int>(shift, '-');
              try {
                EmployeeManager::getInstance()
                  .get(stoi(row[0]))
                  ->getFactor()
...
void
output::employeeRepository(const std::string& path)
{
...
void
output::desiredSchedule(const std::string& path)
{
...
void
output::schedule(const std::string& path)
{
...
void
output::teamRepository(const std::string& path)
{
...
void
output::teamSchedule(const std::string& path)
{
...
void
input::loadDesiredShifts(const std::string& path)
{
  std::ifstream file;
