...
void
EmployeeRepository::checkIfIdExist(const unsigned int& id)
{
...
void
EmployeeRepository::add(const employeePtr& employee)
{
...
void
EmployeeRepository::add(unsigned int id, const std::string& name)
{
  employeePtr employee = std::make_shared<Employee>(name, id);
  employeesRepository.emplace(
    std::make_pair(id, std::make_shared<Employee>(name, id)));
}

void
EmployeeRepository::remove(const unsigned int& id)
{
  employeesRepository.erase(id);
}

std::string
EmployeeRepository::info() const
{
  std::ostringstream out;
  for (const auto& employee : employeesRepository) {
    out << employee.second->getId() << ". " << employee.second->getName()
        << std::endl;
  }
  return out.str();
}
...
employees
EmployeeRepository::getAll() const
{
...
const employeePtr&
EmployeeRepository::get(const unsigned int& id) const
{
  try {
...
employees
EmployeeRepository::getByPosition(const positionPtr& position) const
{
  employees employeesByPosition;
  for (const auto& employee : employeesRepository) {
    if (employee.second->getFactor()->getAuthorisation().positionMatch(
          position)) {
...
EmployeeRepository::getByTeam(const teamPtr& team) const
{
...
employees
EmployeeRepository::getByType(unsigned int typeID) const
{
  employees employeesByType;
  for (const auto& employee : employeesRepository) {
    if (employee.second->getFactor()->getRules().getType()->getPriority() ==
        typeID) {
...
std::string
EmployeeRepository::getStatisticsByID(unsigned int id) const
{
  std::ostringstream out;
...
employees
EmployeeRepository::getAllUnsatisfied() const
{
  employees unsatisfied;
  for (const auto& employee : employeesRepository) {
    if (employee.second->getFactor()->getRules().getMinShifts() >
        employee.second->getFactor()->getAvailability().getShiftsQuantity()) {
