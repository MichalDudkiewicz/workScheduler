...
PositionRepository&
PositionRepository::getInstance()
{
...
void
PositionRepository::add(const positionPtr& position)
{
  positionsRepository.insert(std::make_pair(position->positionID(), position));
}

void
PositionRepository::remove(const unsigned int& id)
{
  positionsRepository.erase(id);
}

const positionPtr&
PositionRepository::get(const unsigned int& id) const
{
  try {
...
positions
PositionRepository::getAll() const
{
