...
void
Team::addShift(shiftPtr& shift)
{
  shifts[shift->getDay() - 1] = std::move(shift);
}

void
Team::addShift(unsigned int startHour, unsigned int endHour, unsigned int day)
{
...
void
Team::removeShift(unsigned int day)
{
...
void
Team::addPosition(const positionPtr& position)
{
...
void
Team::removePosition(const positionPtr& position)
{
  positionsRequired.remove(position);
}

std::string
Team::positionsInfo() const
{
...
std::string
Team::shiftsInfo() const
{
  std::ostringstream out;
  out << "Team shifts: " << std::endl;
  unsigned int weekDay = 0;
  for (const auto& shift : shifts) {
    out << calendar::getWeekDay(weekDay) << ": " << shift->shiftInfo()
        << std::endl;
    weekDay++;
  }
  return out.str();
...
{
  std::ostringstream out;
  out << "Team " << name << std::endl
      << positionsInfo() << std::endl
      << shiftsInfo() << std::endl;
  return out.str();
}
...
const positions&
Team::getPositions() const
{
  return positionsRequired;
}

const std::vector<shiftPtr>&
Team::getShifts() const
{
  return shifts;
}

void
Team::setName(std::string n)
{
  name = std::move(n);
}
