...
TeamQueues::TeamQueues(teamPtr t, const employees& authorisedEmployees)
  : team(std::move(t))
{
      ...
      for (auto& queueToPosition : queuesOnDay) {
        for (const auto& employee : authorisedEmployees) {
          if (employee->getFactor()->getAuthorisation().isAuthorised(
                queueToPosition.first, team) and
              employee->getFactor()->getAvailability().isAvailable(
...
const Calendar<employeesToPosition>&
TeamQueues::getTeamQueues() const
{
  return calendarOfQueues;
}

const teamPtr&
TeamQueues::getTeam() const
{
  return team;
}

void
TeamQueues::queueSort(unsigned int day, const positionPtr& position)
{
  calendarOfQueues[day].at(position).sort(sortPointsTypeWorkHours());
}

std::string
TeamQueues::teamQueuesInfo() const
{
