...
struct FixtureEmployeeTest
{
  ...
  std::shared_ptr<Employee> employee0 =
    std::make_shared<Employee>("Mateusz", 0);
...
BOOST_FIXTURE_TEST_CASE(EmployeeFriendsEnemiesCase, FixtureEmployeeTest)
{
  ...
  BOOST_CHECK_EQUAL(
    (*employee1).getFactor()->getRelationship().getMyFriends().front()->getId(),
    2);
  BOOST_CHECK_EQUAL(
    (*employee0).getFactor()->getRelationship().isFriendWith(employee1), true);
  BOOST_CHECK_EQUAL(
    (*employee1).getFactor()->getRelationship().isFriendWith(employee0), true);
  (*employee1).getFactor()->getRelationship().addEnemy(employee0);
  BOOST_CHECK_EQUAL(
    (*employee1).getFactor()->getRelationship().isFriendWith(employee0), false);
  BOOST_CHECK_EQUAL(
    (*employee0).getFactor()->getRelationship().isFriendWith(employee1), false);
  BOOST_CHECK_EQUAL(
  ...
  BOOST_CHECK_EQUAL(
    (*employee0).getFactor()->getRelationship().isEnemyWith(employee3), false);
  BOOST_CHECK_EQUAL(
    (*employee0).getFactor()->getRelationship().isFriendWith(employee3), false);
}
