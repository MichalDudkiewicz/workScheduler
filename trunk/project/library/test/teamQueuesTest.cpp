...
BOOST_FIXTURE_TEST_CASE(TeamQueuesCaseGetTeamQueues, FixtureTeamQueuesTest)
{
  ...
  BOOST_REQUIRE_EQUAL(
    queue.getTeamQueues()[11].at(team->getPositions().back()).front()->getId(),
    2);
  BOOST_REQUIRE_EQUAL(
...
BOOST_AUTO_TEST_CASE(TemaQueueTestSort)
{
  ...
  std::shared_ptr<Employee> employee1 =
    std::make_shared<Employee>("Paulina", 1);
  ...
  std::shared_ptr<Employee> employee4 =
    std::make_shared<Employee>("Justyna", 4);
  ...
  BOOST_CHECK_EQUAL(queues.getTeamQueues()[d.day() - 1]
                    ...
                    employee3);
  BOOST_CHECK_EQUAL(
    queues.getTeamQueues()[d.day() - 1].at(team->getPositions().front()).back(),
    employee5);
