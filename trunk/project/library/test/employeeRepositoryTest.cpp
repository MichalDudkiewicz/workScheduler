...
BOOST_AUTO_TEST_CASE(EmployeeRepositoryCase)
{
    ...
    BOOST_CHECK_EQUAL(EmployeeRepository::getInstance().get(9)->getName(),
        "Paulina");
    BOOST_TEST_MESSAGE(EmployeeRepository::getInstance().info());
    BOOST_CHECK_EQUAL(
        EmployeeRepository::getInstance().getByPosition(doctor).size(), 2);
    BOOST_CHECK_EQUAL(
        EmployeeRepository::getInstance().getByPosition(driver).size(), 0);
    BOOST_CHECK_EQUAL(EmployeeRepository::getInstance().getByTeam(team)[0].size(),
        2);
    BOOST_CHECK_EQUAL(EmployeeRepository::getInstance().getByTeam(team)[1].size(),
        0);
    ...
    BOOST_CHECK_EQUAL(EmployeeRepository::getInstance().getStatisticsByID(5),
        "ID: 5\n"
        "name: Janusz\n"
        "type: normal\n"
        "points: 0\n"
    ...
    BOOST_CHECK_EQUAL(
        EmployeeRepository::getInstance().getAllUnsatisfied().size(), 2);
...
BOOST_AUTO_TEST_CASE(ExceptionCase)
{
    ...
    BOOST_REQUIRE_THROW(EmployeeRepository::getInstance().add(janusz1),
        std::logic_error);
    EmployeeRepository::getInstance().remove(77);
    BOOST_REQUIRE_THROW(EmployeeRepository::getInstance().get(77),
        std::logic_error);
}
